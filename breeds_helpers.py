...
class BreedsDatasetGenerator():
    ...
    def split_superclass(self, superclass_wnid, Nsubclasses, balanced,
        ...
        # print(f"Nsubclasses: {Nsubclasses}")
        ...
        return class_ranges, split

    def get_superclasses(self, level, Nsubclasses=None,
                         split_type=None, ancestor=None, balanced=True, 
        ...
        Args:
            ...
            balanced (bool): Whether or not the dataset should be
                             balanced over superclasses
            split_type ("good"/"bad"/"rand"/None): If None, superclasses are
        ...
        if Nsubclasses is None:
            if balanced:
                ...
                Nsubclasses = 1 if split_type is None else 2
        ...
        if split_type is None:
            ...
            for sci, sc in enumerate(sorted(superclasses)):
                class_tup, split = self.split_superclass(sc, Nsubclasses=Nsubclasses, 
                                                     balanced=balanced,
                                                     split_type=split_type, rng=rng)
                subclass_splits[0].append(class_tup[0])
                subclass_splits[1].append(class_tup[1])

                subclass_ids[0].append(split[0])
                subclass_ids[1].append(split[1])

        return superclasses, subclass_splits, label_map, subclass_ids
